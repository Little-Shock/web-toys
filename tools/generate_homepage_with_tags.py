...
def generate_project_card(project_path, config):
    ...
    card_html = f"""                <div class="toy-card" data-categories="{' '.join(all_categories)}" data-tags="{' '.join([tag.lower().replace(' ', '-') for tag in tags])}">
...
<body>
    ...
    # 添加标签
    for tag in sorted(tags, key=lambda x: x.get("count", 0), reverse=True):
        html.append(f'                <div class="tag-item" data-tag="{tag["id"]}">{tag["name"]} ({tag["count"]})</div>')
