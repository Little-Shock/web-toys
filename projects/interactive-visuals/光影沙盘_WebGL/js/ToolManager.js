...
class ToolManager {
   ...
   */
  constructor(particleSystem, container, options = {}) {
    ...
    this.params = {
      ...
      pourRate: options.pourRate || 15, // 每帧倾倒的粒子数量
      digRadius: options.digRadius || 40,
      digStrength: options.digStrength || 1.2,
      smoothRadius: options.smoothRadius || 50,
      smoothStrength: options.smoothStrength || 0.8,
      shakeRadius: options.shakeRadius || 60,
      shakeStrength: options.shakeStrength || 0.8,
  ...
  _setupInteractions() {
    ...
    const getCanvasPosition = (clientX, clientY) => {
      ...
      const scaleX = this.particleSystem.bounds.width / rect.width;
      const scaleY = this.particleSystem.bounds.height / rect.height;
  ...
  _pourSand(x, y, colorInfo) {
    ...
    const options = {
      ...
      radius: this.particleSystem.params.particleSize * random(0.9, 1.1), // 随机化粒子大小
      initialSpeed: random(1.5, 2.5), // 增加初始速度
      type: particleType
    };
    
    this.particleSystem.pourSand(x, y, count, options);
  ...
  _digSand(x, y) {
    ...
    const affectedCount = this.particleSystem.digSand(x, y, radius, strength);
  ...
  _smoothSand(x, y) {
    ...
    const affectedCount = this.particleSystem.smoothSand(x, y, radius, strength);
  ...
  _shakeSand(x, y) {
    ...
    const affectedCount = this.particleSystem.shakeSand(x, y, radius, strength);
